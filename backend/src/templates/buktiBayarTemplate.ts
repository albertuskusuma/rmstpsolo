...
import { DataPDFBayar } from "../models/printPdfModels";

export const generateHTMLBuktiBayar = (data: DataPDFBayar): string => {
    const logoPath = path.join(__dirname, "../../public/favicon_smkstpsolo.png");
    const logoBase64 = fs.readFileSync(logoPath, "base64");

    const { header, list_hasil_pemeriksaan, total_harga } = data;
    ...
    <tr>
      ...
      <td style="text-align:right">${formatRupiah(Number(item.harga || 0))}</td>
    </tr>
  `).join("");
  ...
  <html>
    ...
    <body>
      <div class="page">
        ...
        <!-- JUDUL -->
        <div class="form-title">BUKTI PEMBAYARAN</div>
        ...
        <table class="data">
          ...
          <tfoot>
            <tr>
                ...
                <td style="text-align: right; font-weight: bold;">${formatRupiah(Number(total_harga || 0))}</td>
            </tr>
          </tfoot>
        </table>
